...
// IOStatus 값을 표시용 텍스트로 변환
const getStatusText = (status: IOStatus): string => {
  switch (status.ioValue) {
    case '0': return '정상';
    case '1': return '비정상';
    case '2': return '알 수 없음';
    default: return status.ioValue || '정보 없음';
  }
};
  ...
  <div className="bg-gray-50 p-3 rounded space-y-2">
    ...
    <div className="grid grid-cols-2 gap-2 text-sm">
      <div className="flex justify-between">
        <span>상태:</span>
        <span className={getStatusColor(pump.status)}>
          {getStatusText(pump.status)}
        </span>
      </div>
      <div className="flex justify-between">
        <span>오류:</span>
        <span className={getStatusColor(pump.error)}>
          {getStatusText(pump.error)}
        </span>
      </div>
      {pump.remoteSetting && (
        <div className="flex justify-between">
          <span>원격설정:</span>
          <span className={getStatusColor(pump.remoteSetting)}>
            {getStatusText(pump.remoteSetting)}
          </span>
        </div>
      )}
      {pump.operationTime && (
        <div className="flex justify-between">
          <span>가동시간:</span>
          <span>{pump.operationTime.ioValue}</span>
        </div>
      )}
    </div>
  </div>
...
const LightDetails: React.FC<{ device: Light }> = ({ device }) => (
  <div className="space-y-3">
    ...
    <div className="flex justify-between">
      <span>제어 위치:</span>
      <span className={getStatusColor(device.controlPosition)}>
        {getStatusText(device.controlPosition)}
      </span>
    </div>
    <div className="flex justify-between">
      <span>순서:</span>
...
const ElevatorDetails: React.FC<{ device: Elevator }> = ({ device }) => (
  <div className="space-y-3">
    ...
    {device.downMoveStatus && (
      <div className="flex justify-between">
        <span>하강 상태:</span>
        <span className={getStatusColor(device.downMoveStatus)}>
          {getStatusText(device.downMoveStatus)}
        </span>
      </div>
    )}    <div className="flex justify-between">
      <span>오류 상태:</span>
      <span className={getStatusColor(device.error)}>
        {getStatusText(device.error)}
      </span>
    </div>
...
const WaterTankDetails: React.FC<{ device: WaterTank }> = ({ device }) => (
  <div className="space-y-3">
    ...
    <div className="flex justify-between">
      <span>수압:</span>
      <span className={getStatusColor(device.waterPressure)}>
        {getStatusText(device.waterPressure)}
      </span>
    </div>
    <div className="flex justify-between">
      <span>수위:</span>
      <span className={getStatusColor(device.waterLevel)}>
        {getStatusText(device.waterLevel)}
      </span>
    </div>
    <div className="flex justify-between">
      <span>저수위 상태:</span>
      <span className={getStatusColor(device.lowWaterLevelStatus)}>
        {getStatusText(device.lowWaterLevelStatus)}
      </span>
    </div>
    <div className="flex justify-between">
      <span>고수위 상태:</span>
      <span className={getStatusColor(device.highWaterLevelStatus)}>
        {getStatusText(device.highWaterLevelStatus)}
      </span>
    </div>
    <div className="flex justify-between">
      <span>만수위 상태:</span>
      <span className={getStatusColor(device.fullWaterLevelStatus)}>
        {getStatusText(device.fullWaterLevelStatus)}
      </span>
    </div>    {device.pumps && device.pumps.length > 0 && (
      <div>
        <span className="font-medium">펌프 상태:</span>
        <div className="mt-1 space-y-2">
          {device.pumps.map((pump, index) => (
...
const AirPurifierDetails: React.FC<{ device: AirPurifier }> = ({ device }) => (
  <div className="space-y-3">
    ...
    <div className="flex justify-between">
      <span>통신 상태:</span>
      <span className={getStatusColor(device.communicationFailure)}>
        {getStatusText(device.communicationFailure)}
      </span>
    </div>
  </div>
);
