        ...
        <div className="w-9 flex flex-col justify-around items-center gap-6 py-2">
          {floors.map((floor, index) => (
            <div key={`${floor.floorId}-${index}`} className="relative">
              {isHovered && (
                <div className="absolute right-full mr-8 -mt-3 p-2 bg-primary-900/40 backdrop-blur-md rounded-lg shadow-[0_0_10px_rgba(255,255,255,0.1)] z-20 whitespace-nowrap border border-white/20">
                  <div className="text-xs font-bold text-white/90 px-1">{floor.displayName}</div>
                  <div
                    className="absolute right-[-4px] top-1/2 transform -translate-y-1/2 w-0 h-0
                    border-t-[4px] border-b-[4px] border-l-[4px] border-transparent border-l-white/20"
                  ></div>
                </div>
              )}
