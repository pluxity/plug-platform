...
export function useIndoorPoiEvents({
  ...
  useEffect(() => {
    ...
    if (cursorEl && originalCursorRef.current === undefined) originalCursorRef.current = cursorEl.style.cursor
    const setCursor = (value: string | null) => { if (cursorEl) cursorEl.style.cursor = value ?? '' }
    const clearOutline = () => { if (outlinedRef.current) { Effect.Outline.Clear(); outlinedRef.current = undefined } }
    const handlePointerMove = (evt: PoiPointerMoveEvent) => {
      const id = evt?.target?.id
      if (id) {
        if (outlinedRef.current !== id) { Effect.Outline.SetPoiOutline(id as never); outlinedRef.current = id }
        setCursor(hoverCursor)
      } else { clearOutline(); setCursor(originalCursorRef.current || null) }
    }
    Event.AddEventListener('onPoiPointerMove' as never, handlePointerMove as never)
    return () => { Event.RemoveEventListener('onPoiPointerMove' as never, handlePointerMove as never); clearOutline(); setCursor(originalCursorRef.current || null) }
