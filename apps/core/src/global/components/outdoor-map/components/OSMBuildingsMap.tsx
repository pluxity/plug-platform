...
interface OSMBuildingsMapProps { className?: string; children?: React.ReactNode }

const OSMBuildingsMap: React.FC<OSMBuildingsMapProps> = ({ className, children }) => {
  const tilesetUrl = useMemo(() => Cesium.IonResource.fromAssetId(CESIUM_ION_ASSET_ID), []);
  const terrainProvider = useMemo(() => createTerrainProvider(), []);
  return (
    <BaseCesiumViewer className={`${className} w-full h-full`} sceneMode={Cesium.SceneMode.SCENE3D} terrainProvider={terrainProvider}>
      <Scene />
