  ...
  return (
    <div className="grid grid-cols-2 border-t divide-y divide-gray-200 border-x">
      <FacilityForm {...form}>
        ...
        <FacilityFormItem label="path" className="col-span-2">
          <div className='flex gap-2 items-center'>
            <Button
              type="button"
            ...
            <div className="mt-2">
              {facilityData.facility.path && (
                <p>입력된 경로: {facilityData.facility.path}</p>
              )}
            </div>
            <PathInputModal
              isOpen={isPathModalOpen}
              onClose={() => setIsPathModalOpen(false)}
              onSubmit={(path) => onChange("facility.path", path)}
            />
          </div>
        </FacilityFormItem>
        {children}
