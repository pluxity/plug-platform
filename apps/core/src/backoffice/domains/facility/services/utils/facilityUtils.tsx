import { FacilityItem, SortOptions } from "@/backoffice/domains/facility/components/list/CardListType";
import { FACILITY_TYPE_LABELS, FacilityType } from "@/backoffice/domains/facility/types/facilityTypes";
...
export function createEmptyFacilityData(type: FacilityType): {
  facility: { code: string; description: string; name: string };
  floors?: Floor[];
...
export function getEndpointForFacilityType(type: FacilityType): string {
  switch (type) {
    case "buildings":
...
export function getFacilityRoutePath(type: FacilityType, id?: number): string {
  const basePath = `/backoffice/facility/${type}`;
