...
interface UsePermissionResourcesReturn {
...
export const usePermissionResources = () : UsePermissionResourcesReturn => {
    const [resourceTypes, setResourceTypes] = useState<PermissionResourceType[]>([]);
    const [resourceData, setResourceData] = useState<Record<string, ResourceItem[]>>({});
    ...
    return { resourceTypes, resourceData, isLoading, error };
}; 